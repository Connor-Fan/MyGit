...
def runcmd(cmd):
    ...
    try:
        ...
        logger.handle_exception(f'Can not run the cmd of {cmd}! Error: {err}')
        return 1, None, None
...
def read_txt_file(src):
    ...
    if not os.path.exists(src):
        logger.error_msg(f'Can not find the txt file of {src}')
        return 1
    ...
    try:
        ...
        logger.handle_exception(f'Can not read the file of {src}! Error: {err}')
        return 1
...
def read_json_file(src):
    ...
    if not os.path.exists(src):
        logger.error_msg(f'Can not find the json file of {src}')
        return 1
...
def write_json_file(src, json_dict):
    ...
    try:
        ...
        logger.handle_exception(f'Can not write the json file {src}! Error: {err}')
        return True
